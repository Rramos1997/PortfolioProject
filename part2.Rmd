...
##Here I create a training sample and a testing sample, with these we can make predictions
set.seed(385)
sample<-createDataPartition(Comparison$Count,p = 0.75, list = FALSE)
train<-Comparison[sample, ]
test<-Comparison[-sample, ]

trainmodel<-lm(formula = Count ~ Locale + Crime_type, data = Comparison)
summary(trainmodel)
testmodel<-lm(formula = Count ~ Locale + Crime_type, data = Comparison)
summary(testmodel)
